...
- **2026-08-02 · v0.26.0:** Frontier-orchestrator awareness (prompted by first Fable 5-as-orchestrator use): same-model advisory named as a distinct mode in `fable-advisor.md` — a fresh-context check without model diversity; prefer the other advisor when the trigger is about independence (twice-failed approach, unresolvable fork), same-model acceptable for clean-read triggers (overbuild taste, missing assumptions), and report which kind ran. Route selection now counts the orchestrator's own price tier in the cheapest-route calculus: bulk/mechanical work in a frontier-priced main context is itself the expensive route.
- **2026-08-02 · v0.27.0:** Transcript-tax rule, both adapters. The delegation contract doubles as the test: a leg you can write as a fresh, self-contained prompt does not need the transcript, and running it inline charges the whole transcript as overhead. After a compaction or context-pressure warning (the discrete observable signals both hosts have), the default for bounded legs flips to fresh context; judgment, ambiguity resolution, and final integration stay inline regardless. Not mechanically gateable — hardening is the observable trigger plus a calibration line in both adapters recording transcript-tax misses, so the bias is measured through the state system rather than argued from anecdote.
